...
describe Plant do
  ...
  let(:bed) {
	...
	it "utilizes scope for sorting plants by varietal name" do
		plants_by_name = Plant.all.sorted_by_name
    expect(plants_by_name).to eq(plant2)
  end

	it "utilizes scope for sorting plants by varietal germination date" do
		plants_by_date = Plant.all.sorted_by_germination_date
    expect(plants_by_date).to eq(plant2)
  end
